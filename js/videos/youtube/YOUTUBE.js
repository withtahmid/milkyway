class YOUTUBE extends __VIDEO_PLAYER__ {
    ...
    __init__(videoId) {
        if (!videoId) {
            console.error("YOUTUBE __init__ called without videoId");
            return;
        }
        ...
        try {
            document
                ...
                .classList.add("youtube-player");
            this.__player__ = new YT.Player("yt-video-player", {
                height: document
                    .querySelector(".yt-player-container")
    ...
    __destroy__() {
        try {
            document
                .getElementById("video-section")
                .classList.remove("youtube-player");
    ...
    __getCurrentTime__() {
        try {
            return this.__player__.getCurrentTime
                ? this.__player__.getCurrentTime()
                : 0;
        } catch (error) {
            console.error(error);
        }
    ...
    __getPlayerState__() {
        try {
            return this.__player__.getPlayerState
                ? this.__player__.getPlayerState()
                : null;
        } catch (error) {
            console.error(error);
        }
    ...
    __isPaused__() {
        try {
            return this.__player__.getPlayerState
                ? this.__player__.getPlayerState() === 2
                : true;
        } catch (error) {
            console.error(error);
        }

...
async function getYouTubeInfo(videoId) {
    ...
    try {
        ...
        YOUTUBE_MANAGER.infoCache.set(videoId, {
            title,
            thumbnail,
            channelTitle,
        });
...
const YOUTUBE_MANAGER = {
    ...
    createVideoObj: async function (url) {
        ...
        const video = new YOUTUBE_VIDEO_ITEM(
            videoId,
            ROOM.getUsername() ?? "unknown",
            youtubeInfo.title ?? "",
    ...
    currentId: function () {
        return this.queue.length > 0
            ? this.queue[this.queueIndex].__id__
            : null;
    },
    ...
    renderQueueHTML: async function () {
        ...
        document.getElementById("youtube-queue-serial-info").textContent = `${
            this.queueIndex + 1
        } / ${this.queue.length}`;
    },
    ...
    playFromQueueByIdEx: async function (messege) {
        ...
        let index = this.queue.findIndex(
            (video) => video.__id__ === videomsq.__id__
        );
        if (index === -1) {
    ...
    removeFromQueueById: function (__id__) {
        const active_id =
            this.queueIndex >= 0 ? this.queue[this.queueIndex].__id__ : null;
        ...
        this.queueIndex = active_id
            ? this.queue.findIndex((video) => video.__id__ === active_id)
            : -1;
        this.renderQueueHTML();
    },
    removeFromQueueByIdEx: function (messege) {
        ...
        const index = this.queue.findIndex(
            (video) => video.__id__ === messege.video.__id__
        );
        if (index === -1) {
            return;
        }
    ...
    queueInLoop: function () {
        return document
            .getElementById("youtube-loop-queue-btn")
            .classList.contains("youtube-media-button-active");
    },

    toggleLoopQueue: function () {
        document
            .getElementById("youtube-loop-queue-btn")
            .classList.toggle("youtube-media-button-active");
    ...
    setQueueLoop: function (set = true) {
        if (set) {
            document
                .getElementById("youtube-loop-queue-btn")
                .classList.add("youtube-media-button-active");
        } else {
            document
                .getElementById("youtube-loop-queue-btn")
                .classList.remove("youtube-media-button-active");
        }
    },
    ...
    playIndex: async function (index) {
        try {
            await this.beYouTube(this.queue[index].videoId);
        } catch (error) {
            console.error(error);
            return;
        }
    ...
    beYouTube: async function (videoId) {
        if (VIDEO.__getSourceType__() !== "youtube") {
            const status = await new Promise((resolve, reject) => {
                const timeout = setTimeout(() => {
                    reject({
                        error: "Youtube player is not ready on due time",
                    });
                }, 5000);
                VIDEO.beYouTube(videoId);
    ...
    playDirectByUrl: async function (url) {
        const video = await this.createVideoObj(url);
        try {
            await this.beYouTube(video.videoId);
        } catch (error) {
            console.error(error);
            return;
        }
...
function createQueueItemDiv(video, current = false, first, last) {
    ...
    addedBy.textContent =
        video.addedBy == ROOM.getUsername() ? "You" : video.addedBy;
